...
#define mrb_terminal_color_const_get(sym) mrb_const_get(mrb, mrb_obj_value(mrb_terminal_color()), sym)
...
mrb_value color_code_4bit(mrb_state *mrb, mrb_value color, mrb_bool bg_color)
{
    ...
    mrb_value table = mrb_terminal_color_const_get(table_name);
...
mrb_value mrb_value_to_mode_code(mrb_state *mrb, mrb_value mode, mrb_bool fallback)
{
    ...
    if (mrb_symbol_p(mode) || mrb_string_p(mode))
    {
        ...
        mrb_value table = mrb_terminal_color_const_get(MRB_SYM(ANSI_MODE_CODES));
